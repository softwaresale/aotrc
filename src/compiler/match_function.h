...
namespace aotrc::compiler {
    ...
    public:
         ...
         */
        explicit MatchFunction(aotrc::fa::DFA&& dfa, const std::string &label, std::shared_ptr<llvm::Module> &parentModule, const std::shared_ptr<CompilerContext>& ctx);

        llvm::Value *getPatternLengthArg();
        llvm::Value *getPatternArg();
        void build();

        llvm::Function *getMatchFunction() {
            return matchFunction;
        }

        llvm::AllocaInst *getCounterVar() {
            return counterVar;
        }

        llvm::BasicBlock *getInitialBlock() {
            return initialBlock;
        }

        llvm::BasicBlock *getAcceptBlock() {
            return acceptBlock;
        }

        llvm::BasicBlock *getRejectBlock() {
            return rejectBlock;
        }
